...
//	private int mPlayListSelected = -1;
	...
	private TimerTask mTimerTask = new TimerTask() {
		@Override
		public void run() {
			if (mLinearLayoutControlBar.getVisibility() == View.VISIBLE)
			{
	...
	protected void initializeControls() {
		...
		mSurfaceHolderVlc.addCallback(new SurfaceHolder.Callback() {
			public void surfaceCreated(SurfaceHolder holder) {
				createMediaPlayer(false, mMediaPath,//mPlayListArray.get(mPlayListSelected),
						mSurfaceHolderVlc);
			}
		...
		mSurfaceHolderDef.addCallback(new SurfaceHolder.Callback() {
			public void surfaceCreated(SurfaceHolder holder) {
				createMediaPlayer(true, mMediaPath,//mPlayListArray.get(mPlayListSelected),
						mSurfaceHolderDef);
			}
...
/*		String action = intent.getAction();
		...
		mMediaPath = intent.getStringExtra(getResources().getString(R.string.input_file));
...
//						: View.VISIBLE);
//		mImageButtonPrevious
//        .setVisibility((mMediaPath != null) ? View.GONE
//                : View.VISIBLE);
...
//				: View.VISIBLE);
//		mImageButtonNext.setVisibility((mMediaPath != null) ? View.GONE
//              : View.VISIBLE);
...
//		String uri = mPlayListArray.get(mPlayListSelected);
	...
	public void onClick(View v) {
		...
		case R.id.player_button_previous: {
			...
			///////////////////////////////////////
//			mSeekBarProgress.setProgress(mTime - 20);
		    int seektime = 20*1000;
            if (mSurfaceViewVlc.getVisibility() == View.VISIBLE)
            {
                if ((mMediaPlayer.getCurrentPosition() * 1000 - seektime) < 0) {
                    mMediaPlayer.seekTo(0);
                } else {
                    mMediaPlayer.seekTo(mMediaPlayer.getCurrentPosition() * 1000 - seektime);
                }
            }
            else
            {
                if ((mMediaPlayer.getCurrentPosition() - seektime) < 0) {
                    mMediaPlayer.seekTo(0);
                } else {
                    mMediaPlayer.seekTo(mMediaPlayer.getCurrentPosition() - seektime);
                }
            }
			///////////////////////////////////////
...
//			mSeekBarProgress.setProgress(mTime + 20);
                int seektime = 20 * 1000;
                if (mSurfaceViewVlc.getVisibility() == View.VISIBLE) {
                    if ((mMediaPlayer.getCurrentPosition() + (seektime / 1000)) > mMediaPlayer
                            .getDuration()) {
                    ...
                    if ((mMediaPlayer.getCurrentPosition() + seektime) > mMediaPlayer.getDuration()) {
